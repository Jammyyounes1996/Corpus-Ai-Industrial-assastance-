...
- [X] T001 Create backend directory structure with __init__.py files
- [X] T002 [P] Create frontend directory structure with __init__.py files
- [X] T003 [P] Create tests directory structure with __init__.py files
- [X] T004 [P] Create scripts directory
- [X] T005 [P] Create data directory with subdirectories (uploads, audio, images)
- [X] T006 Create .gitkeep file in data/uploads/
- [X] T007 Create .gitkeep file in data/audio/
- [X] T008 Create .gitkeep file in data/images/
...
- [X] T009 Create environment.yml with Python 3.12 and dependencies list
- [X] T010 [P] Create requirements.txt with all pinned dependencies
- [X] T011 [P] Create .env.example with all environment variables including: WHISPER_DEVICE=auto, WHISPER_COMPUTE_TYPE=auto, SECRET_KEY with comment, MAX_PDF_SIZE_MB=100, MAX_AUDIO_SIZE_MB=100, MAX_IMAGE_SIZE_MB=25
- [X] T012 [P] Create .gitignore with Python, environment, data, and IDE patterns
- [X] T013 [P] Create docker-compose.yml with Qdrant v1.12.0 service
- [X] T014 [P] Create README.md with project title and installation placeholder
- [X] T015 Create backend/config/__init__.py
- [X] T016 Create backend/utils/__init__.py
- [X] T017 [P] Create backend/config/settings.py with pydantic-settings class
- [X] T018 [P] Create backend/utils/logging.py with loguru configuration
- [X] T019 Create backend/database/__init__.py
- [X] T020 Create backend/database/migrations/__init__.py
- [X] T021 Create backend/database/migrations/versions/__init__.py
- [X] T022 Create backend/database/database.py with async engine and session factory
- [X] T023 Create backend/database/models/__init__.py
- [X] T024 [P] Create Project model in backend/database/models.py
- [X] T025 [P] Create Chat model in backend/database/models.py
- [X] T026 [P] Create Message model in backend/database/models.py
- [X] T027 [P] Create File model in backend/database/models.py
- [X] T028 [P] Create OCRResult model in backend/database/models.py
- [X] T029 [P] Create Transcript model in backend/database/models.py
- [X] T030 [P] Create AppSettings model in backend/database/models.py
- [X] T031 [P] Create EvaluationResult model in backend/database/models.py
- [X] T032 Add all 10 Index definitions to backend/database/models.py
- [X] T033 Add foreign key constraints to backend/database/models.py
- [X] T035 Create backend/main.py with FastAPI app instance
- [X] T036 Add CORS middleware to backend/main.py for localhost:8501
- [X] T037 Add GET /health endpoint to backend/main.py
- [X] T038 Add startup/shutdown events to backend/main.py
- [X] T039 Add exception handlers to backend/main.py
- [X] T096 Create backend/schemas/health.py with HealthResponse Pydantic model (status, database, qdrant, ollama, version fields)
- [X] T040 Create frontend/__init__.py
- [X] T041 Create frontend/components/__init__.py
- [X] T042 Create frontend/styles/__init__.py
- [X] T043 Create frontend/state/__init__.py
- [X] T044 Create frontend/utils/__init__.py
- [X] T045 [P] Create frontend/styles/load_css.py with CSS injection helper
- [X] T046 [P] Create frontend/styles/main.css with skeleton CSS and design tokens
- [X] T047 Create frontend/state/session.py with session state helpers
- [X] T048 Create frontend/utils/api_client.py with HTTP client skeleton
- [X] T049 Create frontend/app.py with basic page layout (sidebar + tabs)
- [X] T050 [P] Create scripts/init_db.py that creates SQLite database with all tables and default AppSettings row
- [X] T051 [P] Create scripts/setup_Qdrant_collection.py that creates industrial_assistant collection
- [X] T052 [P] Create scripts/generate_secret_key.py that outputs Fernet key
- [X] T053 [P] Create scripts/verify_environment.py that checks all services
...
- [X] T054 Verify environment.yml includes all dependencies from plan.md
- [X] T055 Verify requirements.txt has all dependencies pinned to exact versions
- [X] T056 Verify .env.example documents all required environment variables
- [X] T057 Verify .gitignore excludes environment files, data directories, and Python cache
- [X] T058 Verify docker-compose.yml has correct Qdrant service configuration
- [X] T059 Verify README.md provides clear installation instructions
...
- [X] T060 Verify /health endpoint returns correct JSON format with status field
- [X] T061 Verify health check includes database connection status
- [X] T062 Verify health check includes Qdrant connection status
- [X] T063 Verify health check includes Ollama connection status
- [X] T064 Verify health endpoint returns HTTP 200 when all services available
- [X] T065 Verify health endpoint handles service disconnection gracefully
...
- [X] T066 Verify init_db script creates SQLite database at expected path
- [X] T067 Verify all 8 tables are created: Project, Chat, Message, File, OCRResult, Transcript, AppSettings, EvaluationResult
- [X] T068 Verify all required indexes are present for query optimization
- [X] T069 Verify AppSettings singleton row (id=1) exists with default values
- [X] T070 Verify foreign key constraints are properly defined
...
- [X] T071 Verify setup_Qdrant_collection script creates "industrial_assistant" collection
- [X] T072 Verify collection has dense vectors configured for 768 dimensions
- [X] T073 Verify collection has sparse vector support enabled
- [X] T074 Verify script handles existing collection gracefully
- [X] T075 Verify script provides clear error messages on Qdrant connection failure
...
- [X] T076 Verify pydantic-settings loads all environment variables
- [X] T077 Verify settings class validates configuration at startup
- [X] T078 Verify missing required variables cause startup failure with clear error message
- [X] T079 Verify invalid configuration values are rejected with helpful error messages
- [X] T080 Verify settings module provides type-safe access to all configuration values
...
- [X] T081 Verify frontend/app.py displays sidebar on the left side of the screen
- [X] T082 Verify frontend displays all tab placeholders
- [X] T083 Verify tab navigation switches content correctly
- [X] T084 Verify design tokens (colors, spacing, typography) are applied via main.css
- [X] T085 Verify session state is properly initialized
- [X] T086 Verify CSS is loaded via load_css.py helper function
- [X] T087 Verify api_client.py skeleton provides HTTP client structure
