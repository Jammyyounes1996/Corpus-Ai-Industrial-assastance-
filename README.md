...
Focused generation and web search tests:

```bash
pytest tests/backend/agent/test_generation_and_web_search.py -q
pytest tests/test_answer_mode_phase4.py -q
```
...
CORPUS uses Tavily only when the user asks for updated, current, verified, or source-backed information.

Examples that should trigger web search:

```text
double check أحدث إصدار من معيار OPC UA واديني المصادر
```

```text
what is the latest version of Node.js? cite sources
```
...
* `search_used=true`
* real sources returned
* final answer includes citations or source references

Examples that should not trigger web search:

```text
اشرح طه حسين بالتفصيل في تقرير منظم
```
...
CORPUS supports configurable generation limits.
...
Detailed mode can be triggered by prompts such as:
...
Use `.env.example` for public configuration only.

If any API key was exposed, rotate it immediately.
...
Make sure Vite is started from the `frontend` folder:
